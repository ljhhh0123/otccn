...
main(n, t)
{
    ...
    dstk = strcpy(sym_stk = VirtualAlloc(0x10000000,0x10000,MEM_RESERVE|MEM_COMMIT , PAGE_EXECUTE_READWRITE),
                  " int if else while break return for define main ") + TOK_STR_SIZE;
    glo = VirtualAlloc(0x10100000,0x10000,MEM_RESERVE|MEM_COMMIT , PAGE_EXECUTE_READWRITE);
    ind = prog = VirtualAlloc(0x10200000,0x10000,MEM_RESERVE|MEM_COMMIT , PAGE_EXECUTE_READWRITE);
    vars = VirtualAlloc(0x10300000,0x10000,MEM_RESERVE|MEM_COMMIT , PAGE_EXECUTE_READWRITE);
	printf("sym_stk==%08X dstk==%08X glo==%08X ind==%08X vars==%08X\n",sym_stk,dstk,glo,ind,vars);
    inp();
    next();
    decl(0);
